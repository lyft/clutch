...
func RegisterFeatureFlagAPIHandlerServer(ctx context.Context, mux *runtime.ServeMux, server FeatureFlagAPIServer) error {
	...
	mux.Handle("POST", pattern_FeatureFlagAPI_GetFlags_0, func(w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
		...
		resp, md, err := local_request_FeatureFlagAPI_GetFlags_0(rctx, inboundMarshaler, server, req, pathParams)
...
func RegisterFeatureFlagAPIHandlerClient(ctx context.Context, mux *runtime.ServeMux, client FeatureFlagAPIClient) error {
	...
	mux.Handle("POST", pattern_FeatureFlagAPI_GetFlags_0, func(w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
		...
		resp, md, err := request_FeatureFlagAPI_GetFlags_0(rctx, inboundMarshaler, client, req, pathParams)
		ctx = runtime.NewServerMetadataContext(ctx, md)
		if err != nil {
			runtime.HTTPError(ctx, mux, outboundMarshaler, w, req, err)
			return
...
var (
	pattern_FeatureFlagAPI_GetFlags_0 = runtime.MustPattern(runtime.NewPattern(1, []int{2, 0, 2, 1, 2, 2}, []string{"v1", "featureflag", "getFlags"}, "", runtime.AssumeColonVerbOpt(true)))
)
