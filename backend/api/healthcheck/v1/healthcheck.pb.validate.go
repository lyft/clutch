...
// errors wrapped in HealthcheckRequestMultiError, or nil if none found.
...
// returned by HealthcheckRequest.Validate(true) if the designated constraints
// aren't met.
...
// violated, an error is returned. When asked to return all errors, validation
// continues after first violation, and the result is a list of violation
// errors wrapped in HealthcheckResponseMultiError, or nil if none found.
// Otherwise, only the first error is returned, if any.
...
// errors returned by HealthcheckResponse.Validate(true) if the designated
