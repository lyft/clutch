...
var deleteExperimentsTests = []struct {
...
}{
	{
		...
		ids:  []uint64{1},
		sql:  `UPDATE experiment_run SET execution_time = tstzrange(lower(execution_time), NOW(), '[]') WHERE id = $1 AND (upper(execution_time) IS NOT NULL OR NOW() < upper(execution_time))`,
		args: []driver.Value{1},
	},
}
